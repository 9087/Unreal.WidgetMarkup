...
echo [12/17] StyleSheet (file)...
"%APP%" /WidgetMarkup/Tests/TestStyleSheetFile --project "%UPROJECT%" %EXTRA_ARGS%
if %errorlevel% neq 0 goto :fail

echo [13/17] StyleSheet (inline)...
"%APP%" /WidgetMarkup/Tests/TestStyleSheetInline --project "%UPROJECT%" %EXTRA_ARGS%
if %errorlevel% neq 0 goto :fail

echo [14/17] StyleSheet (inherit + override)...
"%APP%" /WidgetMarkup/Tests/TestStyleSheetOverride --project "%UPROJECT%" %EXTRA_ARGS%
if %errorlevel% neq 0 goto :fail

echo [15/17] ListView + ObservableCollection...
"%APP%" /WidgetMarkup/Tests/TestListView --project "%UPROJECT%" %EXTRA_ARGS%
if %errorlevel% neq 0 goto :fail

echo [16/17] Dynamic add_child / remove_child / get_child...
"%APP%" /WidgetMarkup/Tests/TestDynamicChild --project "%UPROJECT%" %EXTRA_ARGS%
if %errorlevel% neq 0 goto :fail

echo [17/17] Static child widget blueprint + get_child / remove_child...
